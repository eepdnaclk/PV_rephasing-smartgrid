...
for i=1:N_MC
    plot(mean(BFCost_our),'r-', 'Linewidth', 2.0);
    plot(mean(BFCost_def),'b-', 'Linewidth', 2.0);
end
% plot(Best_def.Cost, 'b-', 'Linewidth', 1.5);
legend('GSP-BF','D-BF');
title('Mean cost function (for 20 Monte-Carlo Simulations) for GSP-BF and D-BF when randomely initialized the population');
