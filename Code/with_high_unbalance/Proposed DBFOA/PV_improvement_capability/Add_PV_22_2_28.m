...
for No_NewPV=22:2:30
    ...
    for MC=1:N_MC
        ...
        mean_VUF_re(MC, (No_NewPV/2)-10)  = mean(VUF_re);
        max_VUF_re(MC, (No_NewPV/2)-10)   = max(VUF_re);
        mean_V(MC, (No_NewPV/2)-10)       = mean(mean(V_re(:,1:3)));
        max_V(MC, (No_NewPV/2)-10)        = max(max(V_re(:,1:3)));
        min_V(MC, (No_NewPV/2)-10)        = min(min(V_re(:,1:3))); 
    end
end