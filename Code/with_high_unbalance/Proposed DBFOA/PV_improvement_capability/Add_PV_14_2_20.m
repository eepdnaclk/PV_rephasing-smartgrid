...
for No_NewPV=14:2:20
    ...
    for MC=1:N_MC
        ...
        mean_VUF_re(MC, (No_NewPV/2)-6)  = mean(VUF_re);
        max_VUF_re(MC, (No_NewPV/2)-6)   = max(VUF_re);
        mean_V(MC, (No_NewPV/2)-6)       = mean(mean(V_re(:,1:3)));
        max_V(MC, (No_NewPV/2)-6)        = max(max(V_re(:,1:3)));
        min_V(MC, (No_NewPV/2)-6)        = min(min(V_re(:,1:3))); 
    end
end