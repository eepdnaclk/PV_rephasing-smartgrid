...
%         [pop(i).Cost, UF] 	= CostFunction(pop(i).Position,N_data);
        ...
        if i==1
            ...
            if Max_Loc>=1 && Max_Loc<=14
                NewSol2.Position(N_data.pv_details(:,1)'>=1 & N_data.pv_details(:,1)'<=14) = randi(3, 1, n_NodesA);
            elseif Max_Loc>=15 && Max_Loc<=24
                NewSol2.Position(N_data.pv_details(:,1)'>=15 & N_data.pv_details(:,1)'<=24) = randi(3, 1, n_NodesB);
            elseif Max_Loc>=25 && Max_Loc<=30
                NewSol2.Position(N_data.pv_details(:,1)'>=25 & N_data.pv_details(:,1)'<=30) = randi(3, 1, n_NodesC);
            elseif Max_Loc>=31 && Max_Loc<=38
                NewSol2.Position(N_data.pv_details(:,1)'>=31 & N_data.pv_details(:,1)'<=38) = randi(3, 1, n_NodesD);
            elseif Max_Loc>=39 && Max_Loc<=47
                NewSol2.Position(N_data.pv_details(:,1)'>=39 & N_data.pv_details(:,1)'<=47) = randi(3, 1, n_NodesE);
            elseif Max_Loc>=48 && Max_Loc<=56
                NewSol2.Position(N_data.pv_details(:,1)'>=48 & N_data.pv_details(:,1)'<=56) = randi(3, 1, n_NodesF);
            else
                NewSol2.Position(N_data.pv_details(:,1)'>=57 & N_data.pv_details(:,1)'<=62) = randi(3, 1, n_NodesG);
            end
