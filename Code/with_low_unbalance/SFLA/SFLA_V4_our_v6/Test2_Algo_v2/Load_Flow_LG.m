function [ LF_V_abs, Unbalance_factor] = Load_Flow_LG()
min_power 	= 0.25;   %Minimum power of each load
max_power 	= 5;      %Maximum power of each load
Transformer_rating = 400;
Loading 	= 1;
pv_scale	= 0.7;      %Default=1
load_scale  = 1.0;

%%%%%%%%%% Load Data %%%%%%%%%
Sing_Phase_Load_Power = xlsread('Data.xlsx', 'Sing_Phase_Load_Power');
Sing_Phase_Load_Power = Sing_Phase_Load_Power(:,2:4);

powerfactor = xlsread('Data.xlsx', 'powerfactor');

pv_details = xlsread('Data.xlsx', 'pv_details');
pvbus = pv_details(:,1);
pvkw = pv_details(:,2);
pvphase = pv_details(:,end);
